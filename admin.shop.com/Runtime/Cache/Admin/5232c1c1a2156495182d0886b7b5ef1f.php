    ...
    <div class="list-div" id="listDiv">
        <table cellpadding="3" cellspacing="1">
            <tr>
                ...
                <th>送货方式名字</th><th>运费</th><th>是否支持送货时间</th><th>简介</th><th>是否默认</th><th>是否显示</th>                <th>操作</th>
            </tr>
            <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                    <td width="50px"><?php echo ($row["id"]); ?><input type="checkbox" name="id[]" value="<?php echo ($row["id"]); ?>" class="ids"></td>
                    <td class="first-cell"><span><?php echo ($row["name"]); ?></span></td><td align='center'><?php echo ($row["price"]); ?></td><td align='center'><?php echo ($row["is_time"]); ?></td><td align='center'><?php echo ($row["intro"]); ?></td><td align='center'><?php echo ($row["is_default"]); ?></td><td align="center"><a class="ajax_get" href="<?php echo U('changeStatus',array('id'=>$row['id'],'status'=>1-$row['status']));?>"><img src="http://admin.shop.com/Public/Admin/images/<?php echo ($row["status"]); ?>.gif"/></a></td>                    <td align="center">
